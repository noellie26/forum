...
func GetPost(db *sql.DB, postID string) (structs.Post, error) {
	postQuery := `
			SELECT postID, title, description, imageFileName, vidFilename, creationDate, username, likes, dislikes, edited, timeEdited, approved, reported, report_reason
			FROM posts
			WHERE postID = ?
		`
	...
	if vidFilename.Valid {
        ...
        post.VidFileName = "" // Set a default value when it is NULL
    }

	if timeEdited.Valid {
		post.TimeEdited = timeEdited.Time.Format("20-06-2022 15:04:05")
	} else {
		post.TimeEdited = ""
	}
